...
// Create Product
router.get('/admin/create', authMiddleware, (req, res) => {
...
router.get("/admin/products/:page?", authMiddleware, async (req, res) => {
...
router.get("/admin/suits/:page?", async (req, res) => {
  let page = req.params.page ? Number(req.params.page) : 1;
  let pageSize = 8;
  let products = await Product.find().limit(pageSize).skip((page - 1) * pageSize);
  let totalRecords = await Product.countDocuments();
  let totalPages = Math.ceil(totalRecords / pageSize);
  res.render("pages/main-site-pages/suits", {layout: "suit-layout.ejs",products,page,totalRecords,totalPages
});
});
